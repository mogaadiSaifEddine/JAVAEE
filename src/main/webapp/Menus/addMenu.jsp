...
<body>
        ...
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="${pageContext.request.contextPath}/menus/insert" method="post">
                            <div class="form-group">
                                <label for="menuName">Menu Name (Optional):</label>
                                <input type="text" class="form-control" id="menuName" name="menuName" placeholder="e.g., Lunch Special, Dinner Menu, etc.">
                            </div>
                            ...
                            <div class="form-group">
                                        ...
                                        <div class="card mb-2">
                                            <div class="card-body p-2">
                                                <div class="form-check">
                                                    <input class="form-check-input dish-checkbox" type="checkbox" name="platIds" value="${plat.idPlat}" id="plat${plat.idPlat}">
                                                    <label class="form-check-label" for="plat${plat.idPlat}">
                                                        <strong>${plat.namePlat}</strong> - $<fmt:formatNumber value="${plat.prixPlat}" pattern="#0.00" />
                                                    </label>
                                                    <p class="mb-0 small text-muted">${plat.discPlat}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </c:forEach>
                                ...
                                <c:if test="${empty platList}">
                                    <div class="alert alert-warning">
                                        No dishes available. Please <a href="${pageContext.request.contextPath}/plats/new">add some dishes</a> first.
                                    </div>
                                </c:if>
                            </div>
                            ...
                            <button type="submit" class="btn btn-primary" id="submitBtn" disabled>Create Menu</button>
                            <a href="${pageContext.request.contextPath}/menus/list" class="btn btn-secondary">Cancel</a>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            let dishPrices = {};
            <c:forEach var="plat" items="${platList}">
                dishPrices[${plat.idPlat}] = ${plat.prixPlat};
            </c:forEach>
